...
public class SuperImageView extends ImageView {
	...
	static final int DRAG = 1;// dragging
	static final int ZOOM = 2;// zooming
	static final int ROTATE = 3;// rotating
	static final int ZOOM_OR_ROTATE = 4; // zooming or rotating
	int mode = NONE;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			...
			dist = spacing(event.getX(0), event.getY(0), event.getX(1),
					event.getY(1));
			// If the distance between two point is larger than 10, it will be seen as multi point mode  
			if (dist > 10f) {
				savedMatrix.set(matrix);
	...
	private void doubleClick(float x, float y) {
		...
		float minScale = Math.min((float) viewW / (float) rotatedImageW,
				(float) viewH / (float) rotatedImageH);
		if (curScale <= minScale + 0.01) { // zoom in
